...
{
	float arr[54] = {};
	int id[54] = {401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 417, 418, 419, 420, 421, 422, 423, 424, 425, 426, 427, 428, 429, 430, 431, 432, 433, 434, 436, 437, 439, 440, 441, 442, 444, 445, 446, 447, 449, 450, 451, 452, 453, 454, 248, 219, 234, 202};
	float NeF[54] = {28.2,26.4,24.4,41,24.2,28,22.4,31,19.8,40.2,41.6,26,27,29.2,26.2,35,22.8,24.8,22.6,26.4,20.4,37.4,37,28.4,39.4,27.8,29.6,30.4,29,28.2,25,24,26.6,24.8,36.4,22.8,41.2,35,31,29.6,21,29.6,25.8,21,35.4,26.4,23.8,22.6,30.4,40,18.6,34.2,27.6,32.8};
	float NiF[54] = {22,24,14,26.4,21.4,23.8,22,29.8,19,30,36.6,25,19,18.6,26.6,32,24,24.6,25,24.6,21,31.4,19.8,31.2,34,19.6,28.6,35.4,21.6,17.6,25,25.8,26,21.8,38.2,23.6,30.8,27.8,28.6,25.6,25.4,30,36,26.6,28,26.4,26,12.6,21.4,32.2,30.2,27,23,34.4};
	float TeF[54] = {20,30,30,44,27,34,31,38,34,31,25,24,26,31,30,30,28,26,34,24,26,27,35,18,26,29,27,34,27,35,21,42,24,27,32,25,31,27,29,44,25,20,27,26,38,33,34,29,30,20,31,32,30,34};
	float TiF[54] = {30,28,26,38,28,34,29,19,25,30,18,36,20,36,33,34,32,32,25,30,29,38,23,25,24,32,23,26,19,39,27,35,28,35,31,27,41,40,33,39,30,29,34,26,29,39,28,32,31,32,27,41,33,35};
	float SeF[54] = {27,32,31,43,31,36,34,27,21,32,27,33,28,27,32,28,22,33,24,30,23,22,31,21,30,29,28,26,26,26,21,38,31,28,23,28,32,39,31,38,30,24,22,19,25,21,28,24,20,22,27,29,35,29};
	float SiF[54] = {27,31,31,36,30,37,38,31,29,36,32,33,33,30,37,30,33,34,28,24,22,30,29,24,37,28,27,35,28,37,27,35,36,36,27,28,33,38,29,42,32,30,28,30,30,30,21,41,27,44,25,32,34,27};
	float FeF[54] = {27,24,24,34,33,36,35,30,26,37,32,26,24,25,32,27,25,28,26,23,20,43,35,22,30,27,23,33,23,28,23,33,27,27,25,24,43,41,26,32,27,28,28,21,36,23,22,23,19,16,19,32,38,34};
	float FiF[54] = {20,26,17,32,25,30,19,32,22,40,40,21,15,22,29,24,17,16,25,25,15,28,27,18,32,19,30,22,25,20,24,36,27,22,32,25,25,33,25,33,22,20,9,24,27,26,17,15,10,27,19,26,32,30};
	string FriendName[54] = {"Chanunya Srathongtha","Nattapon Pongkao","Thanachote Wattanakhantijarone","Thanyapisit Buaprakhong","Punnapa Thianchai","Pakawat Kulchot","Varintorn SIthisint","Awirut Pusansa-ad","Kantapon Hiranchatchawal","Kittpot Onnomdee","Jaruwit Singsom","Chananan sudjairak","Yanisa Limjantra","Thalit Chaimunin","Thatsamaphon Boonchuntuk","Panachai Likhitpanyarat","Pawaris Chaideesripraphan","Pimpatcha Singh","Walaiporn Khumplub","Watcharapol Treesatthayasakul","Apiwich Visarnlerdsiri","Kanok Vongsaroj","Kanokwan Rungruengchokchai","Kollawat Ruanghirun","Jittapat Chanyarungroj","Jirapat Jirayanamit","Chaninat Phongam","Chutphon Kanokpannachot","Noppawut Iamwongsri","Nutthanon Thongcharoen","Tuangsub Rattanathai","Thana Prajonkla","Teerapong Pongdarnpet","Nanthawat Sukkam","Nitiphat Unrrom","Puntana Saengchot","Parichart Sae-yhooong","Pongpayom Monplub","Ponragrit Pasurakarn","Pumipat Korncharornpisuit","Rawisada Anuruttikun","Waritnan Panpimyai","Warisara Kanyaprasit","Watchara Buasrirattana","Vitchakorn Charoenying","Sinthon Wilke","Siraphat Ninprasert","Onicha Nitilappool","Alisa Wongcharoensatit","Panicha Kunanuntapong","Sutida Chankaew","Thanphitcha Khuenyotha","Natchanon Sareetee","Kuntida Meekam"};
	string Ptype[54] = {"ENFJ","ISTP","ESTP","ENTJ","ISFJ","ESTP","ESTP","ESFP","ESTP","ENFP","INTP","ESTJ","ENFP","ESTJ","ISTJ","INTP","ISFJ","ESTJ","ESTJ","ESTJ","ENFP","ESFJ","ESFP","INFP","INTP","ESTP","INTP","ISTP","ENTP","ESTJ","ISFJ","ESTJ","INTP","ISFJ","INTP","ISFJ","ESFJ","ISTJ","ESTP","ISTJ","ISFJ","INTP","INFJ","ISTP","INFP","INTP","ISTP","ISTJ","INTP","INTP","ENTP","ENTJ","ISTJ","INFJ"};
	float MyNe = 26.0;
	float MyNi = 17.6;
	float MyTe = 23.0;
	float MyTi = 29.0;
	...
	for (int i = 0; i < 54; i++)
	{
		...
		types[i].Name = FriendName[i];
		types[i].MbtiType = Ptype[i];
	}
	...
	cout << "The First closest friend is " << types[index0].id << " with type " << types[index0].MbtiType << " FriendName " << types[index0].Name << endl;
	...
	cout << "The Second closest friend is " << types[index1].id << " with type " << types[index1].MbtiType << " FriendName " << types[index1].Name << endl;
	...
	cout << "The Third claosest friend is " << types[index2].id << " with type " << types[index2].MbtiType << " FriendName " << types[index2].Name << endl;

	string findMyType0 = types[index0].MbtiType;
	string findMyType1 = types[index1].MbtiType;
	string findMyType2 = types[index2].MbtiType;
